...
public class jsServiceImpl implements jsService {
    ...
    public jsServiceImpl(JsRepository jsRepository, FavoriteStoreRepository favoriteStoreRepository, JtRepository jtRepository) {
    ...
    public JsResponseDto createJs(JsRequestDto jsRequestDto) {
        FavoriteStoreEntity storeId = favoriteStoreRepository.findById(jsRequestDto.getStoreId())
    ...
    public void deleteJs(Integer jtId, String storeId) {
        FavoriteStoreEntity store = favoriteStoreRepository.findById(storeId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "store not found"));
